...
```aipo
# Exemplo canônico de Aipo: structs, invariantes e async
struct Account {
    id
    var balance = 0.0
    created_at
...
impl Account {
    ...
    fn transfer(var self, target: Account, amount: Float) {
        ...
        # Se qualquer invariante falhar, a mutação sofre rollback atômico
        attempt {
            self.balance -= amount
...
# Inferência estrutural automática: Account satisfaz Payable diretamente
interface Payable {
    fn transfer(var self, target: Account, amount: Float)
}
...
async fn process_payment(acc: Payable, amount: Float) {
    ...
    await do {
        timer
        io.println("Processamento concluído com sucesso")
    }
}
```
