...
- **[Interfaces & Contracts](/en/manual/interfaces-and-contracts)**: Structs with immutability by default (`var` for mutability), automatic structural interface subtyping, `init` hooks, `invariant` predicates, and `var self` mutator methods.
- **[Concurrency & Async](/en/manual/async-and-concurrency)**: `async fn`, sequential `await do { ... }`, task combinators (`task.spawn`, `task.sleep`, `task.all`, `task.race`), and the virtual-time scheduler.
